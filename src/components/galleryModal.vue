...
onMounted(()=>{
    console.log('mounted')
        const galleryWrapper = document.getElementById('kul')
...
<template>
    <Teleport to="body" >
        <div v-bind="$attrs"  v-if="isGalleryOpen" ref="wrapperRef" id="kul" class="gallery-wrapper">
            ...
            <div class="photo-preview-wrapper">
                <div v-for="(img,i) in imgSrcList" class="photo-preview-box" :key="img" @click="changeCurrentPhotoIndex(i)">
...
.gallery-wrapper>.photo-preview-wrapper{
    display: flex;
    max-width: 50%;
    overflow-x: auto;
    justify-self: center;
...
.gallery-wrapper>.photo-preview-wrapper>.photo-preview-box{
    width: 52px;
    height: 52px;
